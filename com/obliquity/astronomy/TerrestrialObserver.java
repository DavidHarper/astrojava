...
public class TerrestrialObserver extends EarthCentre {
    ...
    private void geocentreToTopocentre(double time, Vector position, Vector velocity) {
	final double FL    =   1.0 / 298.257; /* Flattening of the earth         */
	final double RE    =         6378.14; /* Radius of Earth in km.           */
	final double OMEGA = 7.2921151467e-5; /* Earth's rotation in radians/second */
	...
	Vector pTopo = new Vector(pcospd * Math.cos(LST),
				  pcospd * Math.sin(LST),
				  psinpd);

	Vector vTopo = (velocity != null) ? new Vector(-OMEGA * pcospd * Math.sin(LST) * 86400.0,
						       OMEGA * pcospd * Math.cos(LST) * 86400.0,
						       0.0) : null;
